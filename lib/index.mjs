...
function buildDeliveryMessage(text, from) {
	return {
		id: randomUUID(),
		role: "user",
		content: [{
...
*/
function tryDeliver(ctx, to, msg) {
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		name: "mailbox_send",
		description: "Send a message to another DSH session's mailbox (cross-session communication) or to the shared board. Use mailbox_inspect first to find the target session id and see who is active. Reply targeting is YOUR decision: send to another session only when that agent's continued work is actually needed (e.g. an approval request). When the task is done, or a user decision or report is required, reply to the user directly in this session instead of mechanically replying to the sender. When you are waiting on another agent's reply, do NOT poll: read their recent messages with mailbox_read within your own turns and decide the next step from what you see — that costs no extra turns and triggers nothing on their side. If the target session is live, the message is ALSO delivered into its chat history as a '[📬 跨会话消息]' entry (visible as a message bubble with marker/timestamp/copy) AND the target agent is woken to process it in an automatic turn and reply. Turns are only ever triggered by a real new message: there is no polling and no auto-read (mailbox_read/mailbox_poll are explicit pull tools), and to=board never wakes anyone, so the reply chain is bounded by actual work and cannot self-loop. Delivered messages are tracked and skipped by mailbox_poll (which never returns them twice). If the target is not live, or for to=board, the message only lands in the store. Messages are persisted under <DSH_HOME>/session-bridge/ and are only visible to the addressed session (plus everyone on the board). Never put API keys or secrets in a message.",
		parameters: {
			to: {
				type: "string",
				required: true,
		...
		async execute(args, exec) {
			...
			return {
				...
				ts: msg.ts,
				delivered,
				...deliveryError !== void 0 ? { deliveryError } : {},
				...delivered ? { note: "delivered into target history and target agent woken to process it (push model)" } : to !== "board" && deliveryError === void 0 ? { note: "target session is not live; message queued for mailbox_poll" } : {}
			};
		}
	}));
	ctx.tools.register(defineTool({
		name: "mailbox_read",
		description: "Pull-only read of the most recent messages of ANY session's mailbox (or the shared board). Unlike mailbox_poll it does NOT consume the read cursor and does NOT wake anyone, so it is safe to call whenever you need to check what another session recently sent or received — e.g. a developer session reading the approver's latest messages before replying, or an approver checking the developer's progress within its own turn while waiting for a reply (read within your own turns; never schedule or repeat reads as polling — that wastes turns and this read never triggers them). Use mailbox_inspect to find session ids; use mailbox_poll to consume your own new messages.",
		parameters: {
			sessionId: {
				type: "string",
				required: true,
	...
	ctx.tools.register(defineTool({
		name: "mailbox_poll",
		description: "Read new messages addressed to THIS session plus new shared-board messages. Messages delivered to a live session are already visible in this chat history and are skipped here; poll returns only messages that were NOT delivered (target was not live, or board traffic). Polling advances a per-session read cursor, so each message is returned exactly once. For a non-consuming look at ANY session's recent messages use mailbox_read instead. Call this at the start of work or when the user asks you to check for messages, especially in a dual-agent workflow (developer session <-> approval session).",
		parameters: {},
		output: {
			schema: { type: "json" },
			render
	...
	ctx.tools.register(defineTool({
		...
		description: "List all known DSH sessions (id, cwd, created, live/persisted, unread message counts) so you can address mailbox_send and see which sessions are active. Also returns your own session id.",
		parameters: {},
		output: {
			schema: { type: "json" },
			render
		...
		async execute(_args, exec) {
			...
			try {
				...
				return {
					ok: false,
					error: "listSessions failed: " + (e?.message ?? String(e))
				};
			}
			const rows = [];
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			return {
				ok: true,
				messages: boardMessages(root, limit)
			};
		}
	}));
}
